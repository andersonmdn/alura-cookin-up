...
export default {
  ...
  data() {
    return {
      todasReceitas: [] as IReceita[]
    };
  },
  async created() {
    this.todasReceitas = await obterReceitas();
  },
  computed: {
    receitasEncontradas(): IReceita[] {
      return this.todasReceitas.filter((receita) => {
        const possoFazerReceita = receita.ingredientes.every((itemLista) => this.ingredientes.includes(itemLista));
